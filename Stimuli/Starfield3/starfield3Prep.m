...
function output = starfield3Prep(Parameters, ScreenData, StimSettings, NumSubframes)
...
for k=1:numRuns     % for each trial
    ...
    inc_yaw = [zeros(1, N-1) deg2rad(P.yaw(k)*ifi)];
    inc_roll = [zeros(1, N-1) deg2rad(P.roll(k)*ifi)];
    inc_pitch = [zeros(1, N-1) deg2rad(P.pitch(k)*ifi)];
    inc_fb = [zeros(1, N-1) P.fb(k)*ifi];
    inc_ud = [zeros(1, N-1) P.ud(k)*ifi];
    inc_rl = [zeros(1, N-1) P.rl(k)*ifi];
