...
for k=1:numRuns     % for each trial
    ...
    for n=1:N   % for each frame  
        ...
        if show_target
            ...
            % Projection
            if show_target
                [scr_target(1), scr_target(2)] =  project3d(target_pos, MP, viewport);
                [scr_start(1), scr_start(2)] =  project3d(target_start, MP, viewport);
                [scr_end(1), scr_end(2)] =  project3d(target_end, MP, viewport);
            else
        ...
        if show_target
            ...
            target_block = scr_target;
            output(k).target_xy{n} = target_block;
%            colour_block = 255*(target_distance-zNear)/zFar*ones(1, 3);
            colour_block = [255 255*(target_distance-zNear)/zFar*[1 1]];
            output(k).target_color{n} = colour_block;
