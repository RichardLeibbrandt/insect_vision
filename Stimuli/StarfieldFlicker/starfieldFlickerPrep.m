...
function output = starfieldFlickerPrep(Parameters, ScreenData, StimSettings, NumSubframes)
...
P.contrast1      = Parameters(11,:);
P.contrast2      = Parameters(12,:);
P.flicker      = Parameters(13,:);
P.t           = Parameters(14,:)*NumSubframes;
...
for k=1:numRuns     % for each trial
    ...
    C1 = P.contrast1(k);
    C2 = P.contrast2(k);
    C1 = 254*round((1-C1)/2)+1;
    C2 = 254*round((1-C2)/2)+1;
...
%         output(k).dotsize{n} = ones(size(distanceRatios));
        ...
        % Colour = greyscale
        clrs = clr(n)*ones(1, length(fx));
        output(k).color{n} = repmat(clrs, 3, 1);
